...
```{r Example Negatives}
allinflow %>% filter(agi < 0 & !is.na(agi)) %>% arrange(agi) %>% select(year, ofips, dfips, return:agi) %>%  head(., n = 10) %>% kable(., caption = "Inflows", format.args = list(big.mark = ","))

alloutflow %>% filter(agi < 0 & !is.na(agi)) %>% arrange(agi) %>% select(year, ofips, dfips, return:agi) %>% head(., n = 10) %>% kable(., caption = "Outflows", format.args = list(big.mark = ","))
```
...
```{r Suppressed}
kable(allinflow %>% group_by(year) %>% summarise(total = n(), Suppressed = sum(return == -1 | is.na(return)), Pct_Suppressed = round(100*Suppressed / total, digits = 2), Minimum = sum(return == 10 & !is.na(return)), Pct_Minimum = round(100*Minimum / total, digits = 2)), format.args = list(big.mark = ","))
```
...
```{r Crosstabs}
kable(xtabs(~year + st_fips_o, filter(allinflow, st_fips_o > 56 | st_fips_o == 0)), format.args = list(big.mark = ","))
```
...
```{r Pre1995}
kable(xtabs(~county_name + cty_fips_o, filter(allinflow, st_fips_o == 63, as.numeric(year) < 1995)), caption = "Pre 1995 Special Codes")
```
...
```{r Foreign}
kable(xtabs(~year + cty_fips_o, filter(allinflow, st_fips_o == 57)), format.args = list(big.mark = ","))
```
