...
```{r Example Negatives}
allinflow %>% filter(agi < 0 & !is.na(agi)) %>% arrange(agi) %>% select(year, ofips, dfips, return:agi) %>%  head(., n = 10) %>% kable(., caption = "Inflows")

alloutflow %>% filter(agi < 0 & !is.na(agi)) %>% arrange(agi) %>% select(year, ofips, dfips, return:agi) %>% head(., n = 10) %>% kable(., caption = "Outflows")
```
...
```{r Suppressed}
kable(allinflow %>% group_by(year) %>% summarise(total = n(), Suppressed = sum(return == -1 | is.na(return)), Pct_Suppressed = round(100*Suppressed / total, digits = 2), Minimum = sum(return == 10 & !is.na(return)), Pct_Minimum = round(100*Minimum / total, digits = 2)))
```
...
```{r Crosstabs}
kable(xtabs(~year + st_fips_o, filter(allinflow, st_fips_o > 56 | st_fips_o == 0)))
```
...
* In 2003, total values for the `98` FIPS (Total Foreign migration) were `r sum(filter(allinflow, year == 2003)$st_fips_o == 98)` but then increased sharply to `r sum(filter(allinflow, year == 2004)$st_fips_o == 98)` in 2004. This change likely demonstrates that the IRS now filled in a value of 0 (or suppressed) across all counties from 2004 and beyond where before the IRS would omit a value for the county if no (or suppressed) migration with foreign areas occurred.

* In 2010, total values for the `57` FIPS (Foreign flows) were `r sum(filter(allinflow, year == 2010)$st_fips_o == 57)` but then increased to `r sum(filter(allinflow, year == 2011)$st_fips_o == 57)` for 2011. I suspect the same implementation occurred as above: the IRS now replaces values with 0 (or suppressed) when before they may have simply omitted any value. This can be seen with the `98` FIPS (Total - Foreign) which changed from `r sum(filter(allinflow, year == 2010)$st_fips_o == 98)` to `r sum(filter(allinflow, year == 2011)$st_fips_o == 98)` across the same time.
...
```{r Foreign}
kable(xtabs(~year + cty_fips_o, filter(allinflow, st_fips_o == 57)))
```
